...
- 2026-06-13: OEM hardware hooks migrated to common (Framework/Ampere setup, theming, SVG icons); Homebrew 6.0 tap trust fixed across system.just and Brewfiles; nomodeset removal dropped as dead code; SYS_ID bug fixed in 10-theming.sh. Cleanup follow-ups filed in bluefin#557 and bluefin-lts#208. ([common#669](https://github.com/projectbluefin/common/pull/669), [common#672](https://github.com/projectbluefin/common/pull/672))
- 2026-06-13: added nvidia-container-toolkit-base + CDI preset to all nvidia image variants (bluefin#559, bluefin-lts#209, common#675); documented in docs/skills/nvidia.md
| 2026-06-13 | CI | Build pipeline fully repaired: (1) rootless buildah→root podman storage gap fixed via sudo skopeo copy promote step (#696); (2) GHCR login added before cosign signing in manifest job (#697); (3) dead downstream dispatch removed — common-updated event had no listeners in bluefin/bluefin-lts/dakota (#701); (4) sync-labels.yml removed (unused). Build, push, sign/SBOM/attest all green. | common#696, common#697, common#701 |
| 2026-06-13 | CI | Renovate automerge unblocked: create-github-app-token v1→v3 (Node.js 24 crypto), reusable automerge now receives mergeraptor bypass token from caller (github-actions[bot] not in ruleset bypass list), lifecycle-caller.yml updated v0→v1. All 7 pending Renovate PRs cleared. | common#688, common#694, common#683 |
| 2026-06-13 | Image | OEM hardware hooks migrated to common: Framework/Ampere system-setup and user-setup hooks, SVG icons. AMD 3.5mm audio jack fix made kernel-aware (Fedora removes obsolete flag; CentOS/RHEL adds it — required for bluefin-lts). ASUS hardware hooks added (asusctl-linux, rog-control-center-linux). | common#672, common#692 |
| 2026-06-13 | Image | Reverted AI slop from #684: nvidia-stacks, amd-stacks, nvidia.just, amd.just, preview-tui Justfile recipe, docs/skills/amd.md, docs/skills/ujust-tui.md erroneously squashed into a feat(report) PR. | common#687 |
| 2026-06-13 | Image | Super+E keybinding restored (Nautilus via custom7 dconf entry); brew-preinstall bats tests + codecov added; Homebrew 6.0 tap trust fixed across all Brewfiles and system.just. | common#690, common#693, common#695 |

...
`lifecycle-caller.yml` and `sync-codeowners.yml` are factory-policy workflows rather than image-test workflows. Lifecycle ownership lives in `projectbluefin/actions/.github/workflows/lifecycle.yml`. The `lifecycle-caller.yml` in each repo is a thin SHA-pinned caller — Renovate keeps that SHA current.
